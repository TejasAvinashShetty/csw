...
Everyone interacting in the csw project's code base,
...
Report bugs at https://github.com/TejasAvinashShetty/csw/issues.
...
The best way to send feedback is to file an issue at https://github.com/TejasAvinashShetty/csw/issues.
...
3. Check https://travis-ci.org/TejasAvinashShetty/csw/pull_requests
...
Ready to contribute? Follow `Aaron Meurer's Git Workflow Notes`_ (with ``TejasAvinashShetty/csw`` instead of ``sympy/sympy``)

In short,

1. Clone the repository from ``git@github.com:TejasAvinashShetty/csw.git``
...
Contributing to the package's developments requires that you have Python y
...
csw uses a simple branching model where all
...
csw includes a full test-suite using pytest_.
We strive for a `test coverage`_ above 90%.


From a checkout of the ``csw`` repository  you can use
...
.. _test coverage: https://coveralls.io/github/TejasAvinashShetty/csw?branch=master
...
csw could always use more documentation, whether
...
At any point, from a checkout of the ``csw``
...
https://github.com/TejasAvinashShetty/csw/releases.
